...
async def ping(e):
    if e.sender_id in SUDO_USERS:
        start = datetime.now()
        altron = await e.reply(f"» __𝗔𝗕𝗘 𝗠𝗘𝗥𝗔 𝗟𝗢𝗗𝗔 𝗠𝗧 𝗖𝗛𝗨𝗦__")
        end = datetime.now()
        mp = (end - start).microseconds / 1000
        await altron.edit(f"__🐆 ʀᴇᴄʀᴇᴀᴛɪᴏɴ ɪs ʀᴇᴀᴅʏ ᴛᴏ ғᴜᴄᴋ ᴜ ᴜᴘ 🗿__\n» `{mp} ᴍꜱ`")
...
async def restart(e):
    if e.sender_id in SUDO_USERS:
        await e.reply(f"`BAPP ABI CHARGE LENE JAA RAHA HAY...`")
        try:
            await X1.disconnect()
        except Exception:
            pass
...
async def addsudo(event):
    if event.sender_id == OWNER_ID:
        ...
        ok = await event.reply(f"» __ᴀᴅᴅɪɴɢ baccha ᴀꜱ ꜱᴜᴅᴏ...__")
        target = ""
